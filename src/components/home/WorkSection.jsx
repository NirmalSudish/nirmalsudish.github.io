...
const ProjectCard = memo(({ item, onMouseEnter, onMouseLeave, onSelect, index }) => {
  ...
  return (
    <div className="project-card flex-shrink-0 relative group/card cursor-pointer" onMouseEnter={() => isProject && onMouseEnter(item.bgColor || '#1d1d1d')} onMouseLeave={onMouseLeave} onClick={() => !isProject && onSelect(item, index)}>
      {isProject ? (
        <Link to={`/project/${item.id}`} className="block transition-all duration-500" style={{ width: 'clamp(280px, 40vw, 600px)' }}>
          <div className="rounded-xl overflow-hidden bg-zinc-900 w-full relative" style={{ height: 'clamp(140px, 28vh, 400px)', marginBottom: 'clamp(0.25rem, 1vh, 0.75rem)' }}>
          ...
          <div className="flex justify-between items-start px-1 w-full">
            <div className="text-left"><h3 className="font-bold uppercase tracking-tighter leading-none" style={{ fontSize: 'clamp(0.75rem, 2vh, 1.5rem)', marginBottom: 'clamp(0.125rem, 0.5vh, 0.25rem)' }}>{item.client}</h3><p className="opacity-60 uppercase tracking-widest font-medium" style={{ fontSize: 'clamp(0.5rem, 0.9vh, 0.625rem)' }}>{item.project}</p></div>
            <div className="text-right"><span className="text-purple-600 dark:text-[#c792ff] font-black uppercase" style={{ fontSize: 'clamp(0.5rem, 0.9vh, 0.625rem)', letterSpacing: '0.15em' }}>{item.categories.join(' / ')}</span></div>
          </div>
        </Link>
      ) : (
        <div className="w-auto rounded-xl overflow-hidden bg-zinc-900 border border-white/5" style={{ height: 'clamp(180px, 28vh, 450px)' }}>
...
const WorkSection = () => {
  ...
  return (
    <section id="work" className="work-section-scaled relative flex flex-col bg-transparent z-10 overflow-hidden" style={{ height: '100vh', paddingTop: 'clamp(60px, 9vh, 100px)', paddingBottom: 'clamp(50px, 8vh, 80px)', justifyContent: 'space-evenly' }}>
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20 text-center" style={{ marginBottom: 'clamp(0.25rem, 1.5vh, 1rem)' }}>
        <ScrollReveal>
          <h2 className="font-black uppercase tracking-tighter leading-none text-black dark:!text-white" style={{ fontSize: 'clamp(1.25rem, 5vh, 4.5rem)', marginBottom: 'clamp(0.25rem, 1vh, 0.75rem)' }}>Featured Work</h2>
        </ScrollReveal>
        <ScrollReveal delay={0.2}>
          <div className="flex flex-wrap justify-center items-center w-full max-w-6xl mx-auto" style={{ gap: 'clamp(0.25rem, 0.8vh, 0.75rem)' }}>
            {[
              ...
              <button key={f.id} onClick={() => { setActiveFilter(f.id); setIsPaused(false); }} className={`group relative flex items-center justify-center rounded-full transition-all duration-500 font-black uppercase tracking-widest transform-gpu ${activeFilter === f.id ? 'bg-black text-white dark:bg-white dark:text-black ring-2 ring-transparent dark:ring-1 dark:ring-white scale-105 shadow-lg shadow-purple-500/30' : 'bg-white/5 ring-2 ring-inset ring-black/10 dark:ring-[0.5px] dark:ring-white/50 text-black dark:!text-white hover:bg-black/5 dark:hover:bg-white/10'}`} style={{ padding: 'clamp(0.25rem, 0.7vh, 0.625rem) clamp(0.5rem, 1.2vw, 1.25rem)', fontSize: 'clamp(0.4rem, 0.9vh, 0.625rem)', gap: 'clamp(0.125rem, 0.5vh, 0.375rem)' }}>
